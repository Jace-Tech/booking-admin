...
export const handleDeleteRoute = async (id) => {
  ...
  try {
    const request = await fetch(
      DELETE_ROUTE_ENDPOINT.replace(":id", id),
      options
    );
...
export const handleUpdateRoute = async (id, data) => {
  let token = localStorage.getItem(STORAGE_NAME);
  if (!token) return { success: false, message: "Failed to fetch", data: null };
  token = JSON.parse(token)["token"];
  ...
  try {
    const request = await fetch(
      UPDATE_ROUTE_ENDPOINT.replace(":id", id),
      options
    );
    const response = await request.json();
    logMessage(response);
...
export const handleCreateBus = async (data, id) => {
  ...
  try {
    const request = await fetch(
      CREATE_ROUTE_BUS_ENDPOINT.replace(":id", id),
      options
    );
    const response = await request.json();
    logMessage(response);
