...
import { handleUpdateBus } from "apis/bus.api";
...
const EditBus = ({ routeId, busEdit, handleFinish }) => {
  const {bool, open, close} = useBoolean()
  const { routes, fetchRoutes, fetchBuses } = useStatsContext();
  const { control, handleSubmit, reset } = useForm({ mode: "onTouched", defaultValues: {
    name: busEdit?.name,
    type: busEdit?.busType,
    boardingDate: new Date(busEdit?.boardingDate),
    seatNumber: busEdit?.seats?.length
  } });
  ...
  const UpdateBus = async (data) => {
    ...
    // MAKE REQUEST
    const result = await handleUpdateBus(busEdit._id, data);
      ...
      <Card className="bg-white shadow">
        <CardHeader className="bg-white border-0">
          <Row className="align-items-center">
            <Col xs="8">
              <h3 className="mb-0">Edit Bus</h3>
            </Col>
          </Row>
        </CardHeader>
        <CardBody>
          <Form onSubmit={handleSubmit(UpdateBus)}>
